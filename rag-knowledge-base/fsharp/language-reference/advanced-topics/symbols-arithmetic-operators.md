...
| `**` (exponentiation, to the power of) | Possible overflow condition when the result exceeds the maximum absolute value for the type. The exponentiation operator works only with floating-point types. |
...
| Binary Operator | Notes |
| --- | --- |
| `+` (addition, plus) | Throws an [OverflowException](/en-us/dotnet/api/system.overflowexception) if the result exceeds the maximum value or goes below the minimum value supported by the type. Both **Overflow** and **Underflow** are possible. |
| `-` (subtraction, minus) | Throws an [OverflowException](/en-us/dotnet/api/system.overflowexception) if the result exceeds the maximum value or goes below the minimum value supported by the type. Both **Overflow** and **Underflow** are possible. |
| `*` (multiplication, times) | Throws an [OverflowException](/en-us/dotnet/api/system.overflowexception) if the product exceeds the maximum value or goes below the minimum value supported by the type. Both **Overflow** and **Underflow** are possible. |
...
All of the operators discussed in this topic are defined in the **Microsoft.FSharp.Core.Operators** namespace. Some of the operators are defined by using statically resolved type parameters. This means that there are individual definitions for each specific type that works with that operator. All of the unary and binary arithmetic and bitwise operators are in this category. The comparison operators are generic and therefore work with any type, not just primitive arithmetic types. Discriminated union and record types have their own custom implementations that are generated by the F# compiler. Class types use the method [Equals](/en-us/dotnet/api/system.object.equals).

The generic operators are customizable. To customize the comparison functions, override [Equals](/en-us/dotnet/api/system.object.equals) to provide your own custom equality comparison, and then implement [IComparable](/en-us/dotnet/api/system.icomparable). The [System.IComparable](/en-us/dotnet/api/system.icomparable) interface has a single method, the [CompareTo](/en-us/dotnet/api/system.icomparable.compareto) method.
...
```
> Reference: https://learn.microsoft.com/en-us/dotnet/fsharp/language-reference/symbol-and-operator-reference/arithmetic-operators
