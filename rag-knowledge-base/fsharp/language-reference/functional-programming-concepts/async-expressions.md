...
*   .NET tasks, [Task<TResult>](/en-us/dotnet/api/system.threading.tasks.task-1) and the non-generic [Task](/en-us/dotnet/api/system.threading.tasks.task), by combining with `Async.AwaitTask`
*   .NET value tasks, [ValueTask<TResult>](/en-us/dotnet/api/system.threading.tasks.valuetask-1) and the non-generic [ValueTask](/en-us/dotnet/api/system.threading.tasks.valuetask), by combining with `.AsTask()` and `Async.AwaitTask`
...
F# async expressions don't support asynchronous `try .. finally ..`. You can use a [task expression](task-expressions) for this case.

## `use` and `use!` bindings
Within async expressions, `use` bindings can bind to values of type [IDisposable](/en-us/dotnet/api/system.idisposable). For the latter, the disposal cleanup operation is executed asynchronously.
...
runAll()
```
> Reference: https://learn.microsoft.com/en-us/dotnet/fsharp/language-reference/async-expressions
