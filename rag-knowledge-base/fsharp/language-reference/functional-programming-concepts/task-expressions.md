# Task expressions
This article describes support in F# for task expressions, which are similar to [async expressions](async-expressions) but allow you to author .NET tasks directly. Like async expressions, task expressions execute code asynchronously, that is, without blocking execution of other work.
...
*   .NET tasks, [Task<TResult>](/en-us/dotnet/api/system.threading.tasks.task-1) and the non-generic [Task](/en-us/dotnet/api/system.threading.tasks.task).
*   .NET value tasks, [ValueTask<TResult>](/en-us/dotnet/api/system.threading.tasks.valuetask-1) and the non-generic [ValueTask](/en-us/dotnet/api/system.threading.tasks.valuetask).
...
Within task expressions, `use` bindings can bind to values of type [IDisposable](/en-us/dotnet/api/system.idisposable) or [IAsyncDisposable](/en-us/dotnet/api/system.iasyncdisposable). For the latter, the disposal cleanup operation is executed asynchronously.
...
## Background tasks
By default, .NET tasks are scheduled using [SynchronizationContext.Current](/en-us/dotnet/api/system.threading.synchronizationcontext.current) if present. This allows tasks to serve as cooperative, interleaved agents executing on a user interface thread without blocking the UI. If not present, task continuations are scheduled to the .NET thread pool.
...
> Reference: https://learn.microsoft.com/en-us/dotnet/fsharp/language-reference/task-expressions
