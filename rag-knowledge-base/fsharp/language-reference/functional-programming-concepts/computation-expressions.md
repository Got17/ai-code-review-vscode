...
If you bind the call to a computation expression with `let`, you will not get the result of the computation expression. Instead, you will have bound the value of the _unrealized_ call to that computation expression. Use `let!` to bind to the result.
...
For the [async workflow](async-expressions), this type is `Async<unit>`. For other computation expressions, the type is likely to be `CExpType<unit>`.

`do!` is defined by the `Bind(x, f)` member on the builder type, where `f` produces a `unit`.

### `yield`

The `yield` keyword is for returning a value from the computation expression so that it can be consumed as an [IEnumerable<T>](/en-us/dotnet/api/system.collections.generic.ienumerable-1):
...
As with the [yield keyword in C#](../../csharp/language-reference/statements/yield), each element in the computation expression is yielded back as it is iterated.
...
When calling a computation expression with `match!`, it will realize the result of the call like `let!`. This is often used when calling a computation expression where the result is an [optional](options).

## Built-in computation expressions
The F# core library defines four built-in computation expressions: [Sequence Expressions](sequences), [Async expressions](async-expressions), [Task expressions](task-expressions), and [Query Expressions](query-expressions).
...
| `Delay` | `(unit -> M<'T>) -> Delayed<'T>` | Wraps a computation expression as a function. `Delayed<'T>` can be any type; commonly `M<'T>` or `unit -> M<'T>` are used. The default implementation returns an `M<'T>`. |
...
| `Run` | `Delayed<'T> -> M<'T>`<br>or<br>`M<'T> -> 'T` | Executes a computation expression. |
| `Combine` | `M<'T> * Delayed<'T> -> M<'T>`<br>or<br>`M<unit> * M<'T> -> M<'T>` | Called for sequencing in computation expressions. |
| `For` | `seq<'T> * ('T -> M<'U>) -> M<'U>`<br>or<br>`seq<'T> * ('T -> M<'U>) -> seq<M<'U>>` | Called for `for...do` expressions in computation expressions. |
...
| `While` | `(unit -> bool) * Delayed<'T> -> M<'T>`<br>or<br>`(unit -> bool) * Delayed<unit> -> M<unit>` | Called for `while...do` expressions in computation expressions. |
...
A computation expression has an underlying type, which the expression returns. The underlying type may represent a computed result or a delayed computation that can be performed, or it may provide a way to iterate through some type of collection. In the previous example, the underlying type was `Eventually<_>`. For a sequence expression, the underlying type is [System.Collections.Generic.IEnumerable<T>](/en-us/dotnet/api/system.collections.generic.ienumerable-1). For a query expression, the underlying type is [System.Linq.IQueryable](/en-us/dotnet/api/system.linq.iqueryable). For an async expression, the underlying type is [`Async`](https://fsharp.github.io/fsharp-core-docs/reference/fsharp-control-fsharpasync-1.html). The `Async` object represents the work to be performed to compute the result. For example, you call [`Async.RunSynchronously`](https://fsharp.github.io/fsharp-core-docs/reference/fsharp-control-fsharpasync.html#RunSynchronously) to execute a computation and return the result.
...
F# computation expressions that suspend execution can be compiled to highly efficient state machines through careful use of a low-level feature called _resumable code_. Resumable code is documented in [F# RFC FS-1087](https://github.com/fsharp/fslang-design/blob/main/FSharp-6.0/FS-1087-resumable-code.md) and used for [Task Expressions](task-expressions).

F# computation expressions that are synchronous (that is, they don't suspend execution) can alternatively be compiled to efficient state machines by using [inline functions](functions/inline-functions) including the `InlineIfLambda` attribute. Examples are given in [F# RFC FS-1098](https://github.com/fsharp/fslang-design/blob/main/FSharp-6.0/FS-1098-inline-if-lambda.md).
...
> Reference: https://learn.microsoft.com/en-us/dotnet/fsharp/language-reference/computation-expressions
