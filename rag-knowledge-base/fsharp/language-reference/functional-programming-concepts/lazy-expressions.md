# Lazy Expressions
_Lazy expressions_ are expressions that are not evaluated immediately, but are instead evaluated when the result is needed. This can help to improve the performance of your code.
...
## Remarks
In the previous syntax, _expression_ is code that is evaluated only when a result is required, and _identifier_ is a value that stores the result. The value is of type `Lazy<'T>`, where the actual type that is used for `'T` is determined from the result of the expression.
...
> Reference: https://learn.microsoft.com/en-us/dotnet/fsharp/language-reference/lazy-expressions
