...
A _nullable value type_ `Nullable<'T>` represents any [struct](structs) type that could also be `null`. This is helpful when interacting with libraries and components that may choose to represent these kinds of types, like integers, with a `null` value for efficiency reasons. The underlying type that backs this construct is [System.Nullable<T>](/en-us/dotnet/api/system.nullable-1).
...
Unlike [Options](options), which are a generalized construct for representing a possible value, nullable value types are not used with pattern matching. Instead, you need to use an [`if`](conditional-expressions-if-then-else) expression and check the `HasValue` property.
...
## Nullable operators
Operations on nullable value types, such as arithmetic or comparison, can require the use of [nullable operators](symbol-and-operator-reference/nullable-operators).
...
> Reference: https://learn.microsoft.com/en-us/dotnet/fsharp/language-reference/nullable-value-types
