...
1.  A scenario is appropriate for an [F# Option](options).
...
## Definition
Value Option is defined as a [struct discriminated union](discriminated-unions#struct-discriminated-unions) that is similar to the reference option type. Its definition can be thought of this way:
...
## Using Value Options
Value Options are used just like [Options](options). `ValueSome` is used to indicate that a value is present, and `ValueNone` is used when a value is not present:
...
As with [Options](options), the naming convention for a function that returns `ValueOption` is to prefix it with `try`.

## Value Option properties and methods
There is one property for Value Options at this time: `Value`. An [InvalidOperationException](/en-us/dotnet/api/system.invalidoperationexception) is raised if no value is present when this property is invoked.
...
> Reference: https://learn.microsoft.com/en-us/dotnet/fsharp/language-reference/value-options
