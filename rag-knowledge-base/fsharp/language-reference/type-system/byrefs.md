...
*   A `byref`\-like struct, which is a [struct](structs) that has similar semantics and the same compile-time restrictions as `byref<'T>`. One example is [Span<T>](/en-us/dotnet/api/system.span-1).
...
> Reference: https://learn.microsoft.com/en-us/dotnet/fsharp/language-reference/byrefs
