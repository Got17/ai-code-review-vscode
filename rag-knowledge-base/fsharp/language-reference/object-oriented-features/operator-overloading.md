...
## Remarks
In the previous syntax, the _operator-symbol_ is one of `+`, `-`, `*`, `/`, `=`, and so on. The _parameter-list_ specifies the operands in the order they appear in the usual syntax for that operator. The _method-body_ constructs the resulting value.

Operator overloads for operators must be static. Operator overloads for unary operators, such as `+` and `-`, must use a tilde (`~`) in the _operator-symbol_ to indicate that the operator is a unary operator and not a binary operator, as shown in the following declaration.
...
_Prefix_ operators are expected to be placed in front of an operand or operands, much like a function. _Infix_ operators are expected to be placed between the two operands.
...
> Reference: https://learn.microsoft.com/en-us/dotnet/fsharp/language-reference/operator-overloading
