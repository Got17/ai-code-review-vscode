...
## Remarks
_Members_ are features that are part of a type definition and are declared with the `member` keyword. F# object types such as records, classes, discriminated unions, interfaces, and structures support members. For more information, see [Records](../records), [Classes](../classes), [Discriminated Unions](../discriminated-unions), [Interfaces](../interfaces), and [Structs](../structs).

Members typically make up the public interface for a type, which is why they are public unless otherwise specified. Members can also be declared private or internal. For more information, see [Access Control](../access-control). Signatures files can also be used to expose or not expose certain members of a type. For more information, see [Signatures](../signature-files).
...
| [Structs](../structs) | Describes the definition of structs in F#. |
| [Explicit Fields](explicit-fields-the-val-keyword) | Describes the definition of uninitialized fields in a type. |

> Reference: https://learn.microsoft.com/en-us/dotnet/fsharp/language-reference/members
