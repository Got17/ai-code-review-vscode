...
Interpolated strings are [strings](strings) that allow you to embed F# expressions into them. They are helpful in a wide range of scenarios where the value of a string may change based on the result of a value or expression.
...
## Format specifiers
Format specifiers can either be printf-style or .NET-style. Printf-style specifiers are those covered in [plaintext formatting](plaintext-formatting), placed before the braces. For example:
...
.NET-style specifiers are those usable with [String.Format](/en-us/dotnet/api/system.string.format), placed after a `:` within the braces. For example:
...
You can also apply formatting that adheres to the rules for [FormattableString](/en-us/dotnet/api/system.formattablestring):
...
Additionally, an interpolated string can also be type checked as a [FormattableString](/en-us/dotnet/api/system.formattablestring) via a type annotation:
...
Note that the type annotation must be on the interpolated string expression itself. F# does not implicitly convert an interpolated string into a [FormattableString](/en-us/dotnet/api/system.formattablestring).
...
Triple quote string literals can start with multiple `$` characters, which changes how many braces are required to open and close interpolation. In these string literals, `{` and `}` characters don't need to be escaped:
...
> Reference: https://learn.microsoft.com/en-us/dotnet/fsharp/language-reference/interpolated-strings
