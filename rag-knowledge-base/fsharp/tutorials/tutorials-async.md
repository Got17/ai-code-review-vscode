...
*   The `Async<'T>` type with [`async { }` expressions](../language-reference/async-expressions), which represents a composable asynchronous computation that can be started to form a task.
*   The `Task<'T>` type, with [`task { }` expressions](../language-reference/task-expressions), which represents an executing .NET task.
...
In the example, the `printTotalFileBytesUsingAsync` function is of type `string -> Async<unit>`. Calling the function does not actually execute the asynchronous computation. Instead, it returns an `Async<unit>` that acts as a _specification_ of the work that is to execute asynchronously. It calls `Async.AwaitTask` in its body, which converts the result of [ReadAllBytesAsync](/en-us/dotnet/api/system.io.file.readallbytesasync) to an appropriate type.
...
Executes a computation in the thread pool. Returns a [Task<TResult>](/en-us/dotnet/api/system.threading.tasks.task-1) that will be completed on the corresponding state once the computation terminates (produces the result, throws exception, or gets canceled). If no cancellation token is provided, then the default cancellation token is used.
...
*   When you need to call into a .NET API that yields a [Task<TResult>](/en-us/dotnet/api/system.threading.tasks.task-1) to represent the result of an asynchronous computation.
...
Returns an asynchronous computation that waits for the given [Task<TResult>](/en-us/dotnet/api/system.threading.tasks.task-1) to complete and returns its result as an `Async<'T>`
...
*   When you are consuming a .NET API that returns a [Task<TResult>](/en-us/dotnet/api/system.threading.tasks.task-1) within an F# asynchronous computation.

What to watch out for:

*   Exceptions are wrapped in [AggregateException](/en-us/dotnet/api/system.aggregateexception) following the convention of the Task Parallel Library; this behavior is different from how F# async generally surfaces exceptions.
...
If using `async { }` programming, you may need to interoperate with a .NET library or C# codebase that uses [async/await](../../csharp/language-reference/keywords/async)\-style asynchronous programming. Because C# and the majority of .NET libraries use the [Task<TResult>](/en-us/dotnet/api/system.threading.tasks.task-1) and [Task](/en-us/dotnet/api/system.threading.tasks.task) types as their core abstractions this may change how you write your F# asynchronous code.
...
To work with APIs that use [Task](/en-us/dotnet/api/system.threading.tasks.task) (that is, .NET async computations that do not return a value), you may need to add an additional function that will convert an `Async<'T>` to a [Task](/en-us/dotnet/api/system.threading.tasks.task):
...
There is already an `Async.AwaitTask` that accepts a [Task](/en-us/dotnet/api/system.threading.tasks.task) as input. With this and the previously defined `startTaskFromAsyncUnit` function, you can start and await [Task](/en-us/dotnet/api/system.threading.tasks.task) types from an F# async computation.
...
> Reference: https://learn.microsoft.com/en-us/dotnet/fsharp/tutorials/async
