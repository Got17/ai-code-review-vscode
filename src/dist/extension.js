...
async function getTransformers(context, mode) {
  ...
  const env2 = module2.env ?? module2.default?.env;
  const pipeline = module2.pipeline ?? module2.default?.pipeline;
  if (!env2 || !pipeline) {
    throw new Error("Transformers.js: env/pipeline not found (ESM interop).");
  }
  const modelsUri = vscode4.Uri.joinPath(context.extensionUri, "models");
  await vscode4.workspace.fs.createDirectory(modelsUri);
  env2.localModelPath = modelsUri.fsPath;
  env2.cacheDir = modelsUri.fsPath;
  env2.allowRemoteModels = mode === "bootstrap";
  return { env: env2, pipeline };
}
var vscode4;
...
async function hasLocalModel(context) {
...
async function downloadModel(context) {
  return vscode5.window.withProgress(
    ...
    async (progress) => {
      ...
      try {
        const { pipeline, env: env2 } = await getTransformers(context, "bootstrap");
...
async function ensureLocalModel(context) {
...
async function getRagContext(context, queryText, topK = DEFAULT_TOP_K) {
  const ragEnabled = vscode5.workspace.getConfiguration("wsCodeReview").get("rag.enable", false);
  if (!ragEnabled) {
    return "";
  }
  try {
    ...
    if (!cacheEntry.extractor) {
      ...
      cacheEntry.extractor = await pipeline("feature-extraction", MODEL_ID);
    }
...
var init_ragContext = __esm({
  "src/utils/ai/ragContext.ts"() {
    ...
    DEFAULT_TOP_K = 5;
    MODELS_DIR = "models";
    MODEL_RELATIVE = ["Xenova", "all-MiniLM-L6-v2"];
    REQUIRED_FILES = [
      "config.json",
      "tokenizer.json",
...
...`;
  ...
  const useRag = vscode6.workspace.getConfiguration("wsCodeReview").get("rag.enable", false);
  if (useRag) {
    try {
